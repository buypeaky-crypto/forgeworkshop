...
import { n as hashedFilesFromTree, o as webseedUrl } from "./webseed-CX85z8gt.mjs";
import { t as createServerRpc } from "./createServerRpc-A6pJPYTF.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/hf-Db7J2h-G.js
...
var UA = { "User-Agent": "Forge/1.1 (Hugging Face model workshop)" };
...
var probeTrust_createServerFn_handler = createServerRpc({
	id: "dc1436535abcc0a47feb3d53645ddcc208efcf0d2560039a5b75dedd919f62f4",
	name: "probeTrust",
	filename: "src/lib/hf.ts"
}, (opts) => probeTrust.__executeServer(opts));
var probeTrust = createServerFn({ method: "GET" }).validator(object({
	id: string().min(1).max(200),
	revision: string().min(1).max(80).optional()
})).handler(probeTrust_createServerFn_handler, async ({ data }) => {
	const id = normalizeId(data.id);
	try {
		const model = asDetail(await hubFetch(`models/${id}`));
		const revision = data.revision?.trim() || model.sha || "main";
		...
		const resolveLive = sample ? await headResolve(webseedUrl(model.id ?? id, revision, sample.path)) : Boolean(model.sha);
		return {
			id: model.id ?? id,
...
var VERIFY_MAX_BYTES = 100663296;
var verifyRemoteFile_createServerFn_handler = createServerRpc({
	id: "5bef9c95793bd409292df9fc4e41553cfc02f004f4218ed05446a7246078c3ce",
	name: "verifyRemoteFile",
	filename: "src/lib/hf.ts"
}, (opts) => verifyRemoteFile.__executeServer(opts));
var verifyRemoteFile = createServerFn({ method: "POST" }).validator(object({
	id: string().min(1).max(200),
	revision: string().min(1).max(80),
	path: string().min(1).max(400),
	expected: string().regex(/^[a-f0-9]{64}$/),
	size: number().int().nonnegative().optional()
})).handler(verifyRemoteFile_createServerFn_handler, async ({ data }) => {
	const id = normalizeId(data.id);
	...
	if ((data.size ?? 0) > VERIFY_MAX_BYTES) {
		...
		return {
			...
			source: "lfs",
			detail: lfsMatch ? "Larger than the in-app fetch window. Official Hub LFS SHA-256 matches. Drop the local shard to hash the bytes." : "Hub LFS SHA-256 does not match the catalog."
		};
	}
	...
	try {
		...
		return {
			...
			source: "remote",
			detail: actual === data.expected ? "Fetched bytes match the official SHA-256." : "Fetched bytes do not match the official SHA-256."
		};
	} catch (err) {
		return {
			path,
...
//#endregion
export { getModel_createServerFn_handler, inspectForVault_createServerFn_handler, listModels_createServerFn_handler, probeTrust_createServerFn_handler, pullSnapshots_createServerFn_handler, scoutHub_createServerFn_handler, verifyRemoteFile_createServerFn_handler };
